/*
Glean Client API

# Introduction These are the public APIs to enable implementing a custom client interface to the Glean system.  # Usage guidelines This API is evolving fast. Glean will provide advance notice of any planned backwards incompatible changes along with a 6-month sunset period for anything that requires developers to adopt the new versions.  # SDK Client bindings for the API can be generated for most popular languages (Python, Java, NodeJS, etc). To do so:  Download the OpenAPI specification for the API, by clicking on one of the following options: 1. [Download JSON specification](https://api.redocly.com/registry/bundle/glean/Glean%20Client%20API%20SDK%20source/v1/openapi.json?branch=main&download=true) 2. [Download YAML specification](https://api.redocly.com/registry/bundle/glean/Glean%20Client%20API%20SDK%20source/v1/openapi.yaml?branch=main&download=true)  Use [openapi-generator-cli](https://github.com/OpenAPITools/openapi-generator-cli) to generate bindings for your language of choice, for example: ```bash shell $ npx @openapitools/openapi-generator-cli@latest generate -i client_api.yaml -g go ```  To see available languages: ```bash shell $ npx @openapitools/openapi-generator-cli@latest list ```  Determine the host you need to connect to. This will be the URL of the backend for your Glean deployment, for example, customer-be.glean.com 

API version: 0.9.0
Contact: support@glean.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package glean_client

import (
	"encoding/json"
)

// checks if the WriteAction type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &WriteAction{}

// WriteAction A single action that can be executed.
type WriteAction struct {
	// The name of the tool.
	ToolName *string `json:"toolName,omitempty"`
	ToolConfig *ToolConfig `json:"toolConfig,omitempty"`
	// If a `REDIRECT` action, the URL to visit to execute the action.
	RedirectUrl *string `json:"redirectUrl,omitempty"`
	// The parameters to be passed to the redirect URL for actions.
	Parameters *map[string]WriteActionParameter `json:"parameters,omitempty"`
}

// NewWriteAction instantiates a new WriteAction object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWriteAction() *WriteAction {
	this := WriteAction{}
	return &this
}

// NewWriteActionWithDefaults instantiates a new WriteAction object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWriteActionWithDefaults() *WriteAction {
	this := WriteAction{}
	return &this
}

// GetToolName returns the ToolName field value if set, zero value otherwise.
func (o *WriteAction) GetToolName() string {
	if o == nil || IsNil(o.ToolName) {
		var ret string
		return ret
	}
	return *o.ToolName
}

// GetToolNameOk returns a tuple with the ToolName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WriteAction) GetToolNameOk() (*string, bool) {
	if o == nil || IsNil(o.ToolName) {
		return nil, false
	}
	return o.ToolName, true
}

// HasToolName returns a boolean if a field has been set.
func (o *WriteAction) HasToolName() bool {
	if o != nil && !IsNil(o.ToolName) {
		return true
	}

	return false
}

// SetToolName gets a reference to the given string and assigns it to the ToolName field.
func (o *WriteAction) SetToolName(v string) {
	o.ToolName = &v
}

// GetToolConfig returns the ToolConfig field value if set, zero value otherwise.
func (o *WriteAction) GetToolConfig() ToolConfig {
	if o == nil || IsNil(o.ToolConfig) {
		var ret ToolConfig
		return ret
	}
	return *o.ToolConfig
}

// GetToolConfigOk returns a tuple with the ToolConfig field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WriteAction) GetToolConfigOk() (*ToolConfig, bool) {
	if o == nil || IsNil(o.ToolConfig) {
		return nil, false
	}
	return o.ToolConfig, true
}

// HasToolConfig returns a boolean if a field has been set.
func (o *WriteAction) HasToolConfig() bool {
	if o != nil && !IsNil(o.ToolConfig) {
		return true
	}

	return false
}

// SetToolConfig gets a reference to the given ToolConfig and assigns it to the ToolConfig field.
func (o *WriteAction) SetToolConfig(v ToolConfig) {
	o.ToolConfig = &v
}

// GetRedirectUrl returns the RedirectUrl field value if set, zero value otherwise.
func (o *WriteAction) GetRedirectUrl() string {
	if o == nil || IsNil(o.RedirectUrl) {
		var ret string
		return ret
	}
	return *o.RedirectUrl
}

// GetRedirectUrlOk returns a tuple with the RedirectUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WriteAction) GetRedirectUrlOk() (*string, bool) {
	if o == nil || IsNil(o.RedirectUrl) {
		return nil, false
	}
	return o.RedirectUrl, true
}

// HasRedirectUrl returns a boolean if a field has been set.
func (o *WriteAction) HasRedirectUrl() bool {
	if o != nil && !IsNil(o.RedirectUrl) {
		return true
	}

	return false
}

// SetRedirectUrl gets a reference to the given string and assigns it to the RedirectUrl field.
func (o *WriteAction) SetRedirectUrl(v string) {
	o.RedirectUrl = &v
}

// GetParameters returns the Parameters field value if set, zero value otherwise.
func (o *WriteAction) GetParameters() map[string]WriteActionParameter {
	if o == nil || IsNil(o.Parameters) {
		var ret map[string]WriteActionParameter
		return ret
	}
	return *o.Parameters
}

// GetParametersOk returns a tuple with the Parameters field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WriteAction) GetParametersOk() (*map[string]WriteActionParameter, bool) {
	if o == nil || IsNil(o.Parameters) {
		return nil, false
	}
	return o.Parameters, true
}

// HasParameters returns a boolean if a field has been set.
func (o *WriteAction) HasParameters() bool {
	if o != nil && !IsNil(o.Parameters) {
		return true
	}

	return false
}

// SetParameters gets a reference to the given map[string]WriteActionParameter and assigns it to the Parameters field.
func (o *WriteAction) SetParameters(v map[string]WriteActionParameter) {
	o.Parameters = &v
}

func (o WriteAction) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o WriteAction) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ToolName) {
		toSerialize["toolName"] = o.ToolName
	}
	if !IsNil(o.ToolConfig) {
		toSerialize["toolConfig"] = o.ToolConfig
	}
	if !IsNil(o.RedirectUrl) {
		toSerialize["redirectUrl"] = o.RedirectUrl
	}
	if !IsNil(o.Parameters) {
		toSerialize["parameters"] = o.Parameters
	}
	return toSerialize, nil
}

type NullableWriteAction struct {
	value *WriteAction
	isSet bool
}

func (v NullableWriteAction) Get() *WriteAction {
	return v.value
}

func (v *NullableWriteAction) Set(val *WriteAction) {
	v.value = val
	v.isSet = true
}

func (v NullableWriteAction) IsSet() bool {
	return v.isSet
}

func (v *NullableWriteAction) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWriteAction(val *WriteAction) *NullableWriteAction {
	return &NullableWriteAction{value: val, isSet: true}
}

func (v NullableWriteAction) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWriteAction) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


